...
def generate_image(input):
    image = pipe(prompt=input, width=width, height=height, num_inference_steps=generation_steps, guidance_scale=cfg, num_images_per_prompt=batch_size, negative_prompt=negative_prompt).images[0]
    buffer = io.BytesIO()
